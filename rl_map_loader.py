...
class RLMapLoader(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            title_frame, text="⚽  RL Map Loader",
        ...
        self.status_label = ctk.CTkLabel(
            self.status_frame, text="  No map loaded  ",
            font=ctk.CTkFont(size=12), text_color=C_TEXT_DIM
        )
        self.status_label.pack(padx=12, pady=4)

        # ── Settings bar ──
        settings = ctk.CTkFrame(self, fg_color=C_CARD, corner_radius=10, border_width=1, border_color=C_BORDER)
        ...
        ctk.CTkButton(
            rl_row, text="Browse", width=90, height=30,
        ...
        ctk.CTkLabel(
            maps_row, text="Maps Folder:", width=120, anchor="w",
            font=ctk.CTkFont(size=13, weight="bold"), text_color=C_TEXT
        ).pack(side="left")

        self.maps_path_var = ctk.StringVar(value=self.config_data.get("maps_folder", ""))
        ctk.CTkEntry(
            maps_row, textvariable=self.maps_path_var, height=30,
            fg_color=C_BG, border_color=C_BORDER, text_color=C_TEXT,
            placeholder_text="Select folder with custom maps..."
        ).pack(side="left", fill="x", expand=True, padx=(8, 8))

        ctk.CTkButton(
            maps_row, text="Browse", width=90, height=30,
            fg_color=C_ACCENT, hover_color=C_ACCENT_HOVER,
        ...
        self.restore_btn = ctk.CTkButton(
            btn_row, text="🔄  Restore Original", height=34,
            fg_color=C_DANGER, hover_color="#c1121f",
        ...
        ctk.CTkButton(
            btn_row, text="🔃  Refresh", height=34,
            fg_color="#2a2e3d", hover_color="#363b4f",
            font=ctk.CTkFont(size=13),
            command=self._refresh_maps
        ).pack(side="left", padx=(10, 0))

        ctk.CTkButton(
            btn_row, text="📂  Open RL Maps", height=34,
            fg_color="#2a2e3d", hover_color="#363b4f",
            font=ctk.CTkFont(size=13),
        ...
        self.maps_count_label = ctk.CTkLabel(
            self, text="Maps (0)",
            font=ctk.CTkFont(size=15, weight="bold"), text_color=C_TEXT, anchor="w"
        )
        ...
        for entry in sorted(os.scandir(maps_folder), key=lambda e: e.name.lower()):
            ...
            for f in os.scandir(entry.path):
                if f.is_file():
                    ...
                    if any(low.endswith(ext) for ext in THUMB_EXTENSIONS):
                        if thumb_file is None or low.startswith(entry.name.lower().replace(" ", "_")):
                            thumb_file = f.path
            ...
            if upk_file:
                found.append({
                    ...
                    "upk": upk_file,
                    "thumb": thumb_file,
                    "upk_size_mb": round(os.path.getsize(upk_file) / (1024 * 1024), 1),
                })
    ...
    def _refresh_maps(self):
        ...
        if not self.maps:
            ...
            if self.maps_path_var.get():
                self.placeholder.configure(text="No maps found in the selected folder (.upk)")
            else:
                self.placeholder.configure(text="Set Rocket League path and maps folder above to get started ☝️")
            return
    ...
    def _create_map_card(self, map_data):
        ...
        card = ctk.CTkFrame(
            self.maps_scroll, fg_color=C_CARD, corner_radius=10,
            border_width=2,
        ...
        # ── Thumbnail ──
        thumb_frame = ctk.CTkFrame(card, fg_color="#000000", corner_radius=8, width=140, height=80)
        ...
        if map_data["thumb"]:
            try:
                ...
                ctk.CTkLabel(thumb_frame, text="🗺️", font=ctk.CTkFont(size=24), text_color=C_TEXT_DIM).pack(expand=True)
        else:
            ctk.CTkLabel(thumb_frame, text="🗺️", font=ctk.CTkFont(size=24), text_color=C_TEXT_DIM).pack(expand=True)
        ...
        if is_active:
            ctk.CTkLabel(
                ...
                text_color=C_SUCCESS, fg_color=C_ACTIVE_BADGE, corner_radius=4
        ...
        ctk.CTkLabel(
            ...
            font=ctk.CTkFont(size=10), text_color="#5a5e72", anchor="w"
        ).pack(anchor="w")

        # ── Button (right) ──
        if is_active:
            btn = ctk.CTkButton(
                card, text="✓  Loaded", height=38, width=110,
                fg_color=C_SUCCESS, hover_color="#25a244",
                font=ctk.CTkFont(size=13, weight="bold"),
                state="disabled"
            )
        else:
            btn = ctk.CTkButton(
                card, text="▶  Load Map", height=38, width=110,
                fg_color=C_ACCENT, hover_color=C_ACCENT_HOVER,
                font=ctk.CTkFont(size=13, weight="bold"),
    ...
    def _restore_original(self):
        ...
        if not os.path.exists(backup):
            messagebox.showinfo("Info", "Original map is already active (no backup found).")
            return
